// =============================================================================
// Updated TripDetailsScreen.tsx with Backend Integration and i18next
...
const TripDetailsScreen: React.FC<TripDetailsScreenProps> = ({
  ...
  const loadActiveTrip = async () => {
    try {
      ...
      setError(error.message || t('errors.failedToLoadTrip'));
      toast.error(t('errors.failedToLoadTrip'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCompleteTrip = async () => {
    ...
    if (!confirm(t('trip.confirmComplete'))) {
      return;
    }
    
    try {
      await tripService.completeTrip(trip._id);
      toast.success(t('trip.tripCompleted'));
      if (onTripCompleted) {
        onTripCompleted();
      }
      onBack();
      ...
      toast.error(error.message || t('trip.failedToComplete'));
    }
  };
  ...
  const handleArchiveTrip = async () => {
    ...
    if (!confirm(t('trip.confirmArchive'))) {
      return;
    }
    
    try {
      await tripService.archiveTrip(trip._id);
      toast.success(t('trip.tripArchived'));
      if (onTripCompleted) {
        onTripCompleted();
      }
      onBack();
      ...
      toast.error(error.message || t('trip.failedToArchive'));
    }
  };
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: trip.name,
        text: `${t('common.view')} ${t('trip.tripDetails')}: ${trip.name} ${t('common.to')} ${trip.destination}`,
        url: window.location.href
      });
    } else {
      navigator.clipboard.writeText(`${trip.name} ${t('common.to')} ${trip.destination}`)
        .then(() => toast.success(t('success.locationShared')))
      ...
      <div className="space-y-6">
        ...
        <div className="px-4 flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-gray-600">{t('home.loadingTripDetails')}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="space-y-6">
        ...
        <div className="px-4 flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('home.noActiveTrip')}</h3>
            <p className="text-gray-600 mb-4">{error || t('trip.noTripDetails')}</p>
            <button
              onClick={onBack}
    ...
    <div className="space-y-6">
      ...
      <div className="px-4 space-y-6">
        ...
        <div className="flex bg-gray-100 rounded-2xl p-1">
          {[
            { key: 'overview' as const, label: t('trip.overview'), icon: FileText },
            { key: 'members' as const, label: t('trip.members'), icon: Users },
            { key: 'itinerary' as const, label: t('trip.itinerary'), icon: MapPin }
          ].map((tab) => (
            <button
              key={tab.key}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-3 flex items-center space-x-2">
                ...
                <span>{t('trip.travelDates')}</span>
              </h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('trip.departure')}:</span>
                  <span className="font-medium">{formatDate(trip.startDate)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('trip.return')}:</span>
                  <span className="font-medium">{formatDate(trip.endDate)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('trip.duration')}:</span>
                  <span className="font-medium">{tripDuration} {t('trip.days')}</span>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-medium px-2 py-1 rounded-full text-xs ${
                    ...
                    {t(`trip.${trip.status}`, trip.status.charAt(0).toUpperCase() + trip.status.slice(1))}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-center">
                ...
                <p className="text-sm text-gray-600">{t('trip.travelers')}</p>
              </div>
              <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-center">
                <MapPin className="text-green-600 mx-auto mb-2" size={24} />
                <p className="text-2xl font-bold text-gray-900">{trip.itinerary.length}</p>
                <p className="text-sm text-gray-600">{t('trip.destinations')}</p>
              </div>
            </div>

            {/* Trip Progress */}
            <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
              <h3 className="font-semibold text-gray-900 mb-3">{t('home.tripProgress')}</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    <Clock className="text-blue-600" size={20} />
                    <span className="text-blue-800 font-medium">
                      {daysRemaining > 0 ? `${daysRemaining} ${t('trip.daysRemaining')}` : t('trip.completed')}
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-green-800 font-medium">{progress}% {t('trip.completed').toLowerCase()}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div key={member.id || index} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600">{t('trip.age')}: {member.age}</p>
                    </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">{t('trip.documentLabel')}</span>
                    ...
                    {member.phoneNumbers && member.phoneNumbers.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-gray-600">{t('trip.phoneLabel')}</span>
                        {member.phoneNumbers.map((phone, phoneIndex) => (
                          <div key={phoneIndex} className="flex justify-between items-center">
                            ...
                            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                              {t(`trip.phoneType.${phone.type}`, phone.type)}
                            </span>
                          </div>
                        ))}
                      </div>
                    ...
                    {member.emergencyContact && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">{t('trip.emergencyContactLabel')}</span>
                        <span className="font-medium">{member.emergencyContact}</span>
                      </div>
                    )}
                    {member.relation && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">{t('trip.relationLabel')}</span>
                        <span className="font-medium">{member.relation}</span>
                      </div>
                    )}
                    {member.specialNeeds && (
                      <div className="mt-2 p-2 bg-orange-50 rounded-lg">
                        <span className="text-gray-600">{t('trip.specialNeedsLabel')}</span>
              ...
              trip.itinerary.map((day, index) => (
                <div key={day.id || index} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-gray-900 text-lg">{t('trip.day')} {index + 1}</h3>
                  ...
                  {day.activities && day.activities.length > 0 && (
                    <div className="space-y-2 mb-3">
                      <h4 className="font-medium text-gray-800">{t('trip.activitiesLabel')}</h4>
